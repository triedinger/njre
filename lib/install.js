 ...
 * @param {string} [options.cache = false] - use true, to use a previously downloaded jre (expires after 2 days currently)
...
function install(version = 11, options = {}) {
  ...
  options = { ...options, openjdk_impl, release, type, heap_size, vendor, cache };
  ...
  if (!options.arch) {
    ...
    else return Promise.reject(new Error(`Unsupported architecture: ${process.arch}`));
  }
  ...
  const cacheFile = path.join(path.dirname(require.main.filename), crypto.createHash("sha256").update(url).digest("hex"));
  ...
  if (options.cache) {
    try {
      ...
      if (cacheStats && cacheStats.isFile()) {
        const msSinceModified = (Date.now() - cacheStats.mtimeMs);

        const twoDays = 60*60*24*2*1000;
        if (msSinceModified>0 && msSinceModified<twoDays) {
          try {
            const cachedDir = fs.readFileSync(cacheFile, 'utf8');

            if (fs.existsSync(cachedDir)) {
              return new Promise((resolve) => resolve(cachedDir));
            }
          } catch (err) {
            console.info('Error reading cache-file: ', err);
          }
        }
      }
    } catch (err) {
      console.info('Error accessig cache-file: ', err);
    }
  }
  ...
  return fetch(url, { redirect: "manual" })
    ...
    .then(extract)
    .then(async (dir) => {
      // create a file with the hash of the url as name, the target folder as the content
      // and save it locally
      try {
        await fs.promises.writeFile(cacheFile, dir);
      } catch (err) {
        console.info('Error writing cache-file: ', err);
      }
